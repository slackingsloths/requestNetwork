# Payment Network - ERC20 Stream - proxy contract
...
This extension allows the payments to be made in ERC20 tokens as money streams on the Ethereum blockchain. Refunds can be made through an ERC20 proxy contract.
The ERC-1620 stream is created through a proxy contract. This proxy contract initiates the stream on behalf of
the user. The contract ensures a link between a streamId and a request through a `paymentReference`.
...
The smart contract is used to check the payment stream completion.

It contains one function called `streamFromWithReference` which takes 7 arguments:

- `streamContractAddress` is the address of the ERC-1620 contract
...
- `startTime` is the timestamp when the ERC20 stream should start
- `stopTime` is the timestamp when the ERC20 stream should end
- `paymentReference` is the reference data used to track the transfer (see `paymentReference`)

The `StreamWithReference` event is emitted when the tokens are transfered. This event contains 3 arguments from the `streamFromWithReference` function:

- `streamContractAddress`
- `streamId`
- `paymentReference`

[See smart contract source](https://github.com/RequestNetwork/requestNetwork/blob/master/packages/smart-contracts/src/contracts/ERC1620Proxy.sol)
...
TODO: from this point, specs are not up-to-date

| Property                  | Type   | Description                                    | Requirement   |
| ------------------------- | ------ | ---------------------------------------------- | ------------- |
| **id**                    | String | constant value: "pn-erc1620-proxy-contract"      | **Mandatory** |
...
| **values.streamContractAddress** | String | Ethereum address for the stream contract       | Optional      |
...
Note: to use the Rinkeby testnet, create a request with `currency.network` as `rinkeby`. TODO Ask Vincent !!!
...
Any `StreamWithReference` events emitted from the streaming proxy contract with the following arguments are considered as a payment:
